  ...
  <div>
    <div>
      <Modal
        ...
        <p slot="header">
          <span>Btbtdy电影转移 {{form.region_name}} <a :href="form.href" target="_blank">{{form.title}}</a></span>
        </p>
        <div>
          <!--同步修改电影下载链接-->
          ...
          <br>
          <Form ref="btbtdymovieadd" :model="form" :label-width="90" :rules="AddRule" class="node-add-form">
...
//          ]
    ...
    methods: {
      add() {
        this.$refs.btbtdymovieadd.validate((valid) => {
          if (valid) {
            this.modal_loading = true;
            let data = this.form;
            this.apiPost('btbtdymovieadd', data).then((res) => {
      ...
      deletedownload(params) {
        ...
        this.apiDelete('btbtdydownloadlink/', id).then((res) => {
          this.handelResponse(res, (data, msg) => {
            this.$Message.success(msg)
      ...
      getbtbtdy(id) {
        this.apiGet('btbtdy/' + id).then((res) => {
          this.handelResponse(res, (data, msg) => {
