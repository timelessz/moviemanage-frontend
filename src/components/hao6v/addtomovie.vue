<template>
  <div>
    <div>
      <Modal
        v-model="modal" width="1000" :transfer="transfer">
        <p slot="header">
          <span>Hao6v电影转移 {{form.region_name}} <a :href="form.href" target="_blank">{{form.title}}</a></span>
        </p>
        <div>
          <!--同步修改电影下载链接-->
          ...
          <br>
          <Form ref="hao6vmovieadd" :model="form" :label-width="90" :rules="AddRule" class="node-add-form">
            ...
            <Row>
              ...
              <div style="width: 85%;margin: 0 auto; padding-top:10px ">
                <Row>
                  <Col span="24">
                  <Select ref="movieregion" v-model="form.region_id" label-in-value @on-change="regionchange">
...
//          ]
    ...
    methods: {
      add() {
        this.$refs.hao6vmovieadd.validate((valid) => {
          if (valid) {
            this.modal_loading = true;
            let data = this.form;
            this.apiPost('hao6vmovieadd', data).then((res) => {
      ...
      deletedownload(params) {
        ...
        this.apiDelete('hao6vdownloadlink/', id).then((res) => {
          this.handelResponse(res, (data, msg) => {
            this.$Message.success(msg)
            this.gethao6v(params.row.movie_id)
          }, (data, msg) => {
            this.$Message.error(msg);
          })
        }, (res) => {
